# Face-tracked piano waterfall design

## Objective

Render a world-stable three-lane piano runway on G2 using only the stationary
Pixel’s front camera and on-device face/head-pose estimation. The wearer needs
no printed marker, external tracker, laptop, or network server. The 16-beat Hot
Cross Buns sequence advances on a 60 Hz monotonic animation clock.

## Core decisions

| Area | Decision | Reason |
|---|---|---|
| Tracking runtime | Native Android APK | CameraX and head-pose processing run continuously with explicit lifecycle control. |
| Display runtime | Even Hub eHPK | The official phone host exclusively owns the G2 connection. |
| APK/eHPK bridge | Loopback HTTP on `127.0.0.1` | Both packages run on one phone; no LAN or laptop dependency. |
| Background lifetime | Camera foreground service | Switching to Even Hub does not pause face inference or the frame server. |
...
| Scheduling | 60 Hz render, newest-frame bridge, conflated eHPK queue | Phone rendering never blocks on G2 and obsolete frames are discarded. |
...
- Set strike places the runway along current gaze;
...
| Runway trails motion | Camera/model/G2 latency | Measure source-frame age and G2 transfer time, then tune bounded prediction. |
| Whole world shifts | Phone stand moved | Use a rigid weighted stand. |
| eHPK shows tracker offline | APK service is stopped or camera permission was removed | Open G2 Piano Tracker, restore permission, calibrate, then return to Even Hub. |
| G2 disconnects | Native direct-BLE debug was enabled | Disconnect it; Even Hub must be the sole G2 owner. |
